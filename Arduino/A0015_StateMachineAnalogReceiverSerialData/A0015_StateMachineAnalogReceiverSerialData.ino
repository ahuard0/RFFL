...
void loop() {
  ...
  delay(50);  // Necessary to avoid lag (was 50 ms)
}
...
void broadcastPinData() {
  ...
  checksum = computeChecksum(outputString);
  
  outputString += "|";
  outputString += checksum;

  for (int i = 0; i < outputString.length(); i++) {
    Serial.print(outputString.charAt(i));
  }
  Serial.println("");
}
